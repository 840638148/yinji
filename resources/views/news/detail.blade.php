  ...
  <div class="cat-wrap left mt20">
      ...
      <ul>
          ...
          <p>{{get_article_title($next_news)}}</p>
            ...
            <div style="margin-bottom:100px;" class="new_article"> <img src="{{get_article_thum($new_news[0])}}"   alt="{{get_article_title($new_news[0])}}" /> <a href="/news/detail/{{$new_news[0]->id}}">{{trans('article.latest')}}</a>
            ...
            <div class="item author-info">
                <div class="users"> 
                    @if ($designer)
                        ...
                        @if($is_subscription) 
                            <span class="Button have-disalbed designer" url="#" title="设计师订阅" designer_id="{{$designer->id}}">已订阅 </span> 
                        @else 
                            <span class="Button3 wpfp_designer_act designer" url="#" title="设计师订阅" designer_id="{{$designer->id}}">订阅</span> 
                        @endif
                    @endif
    ...
    <div class="create_folder modal" id="discoveryFolders_1">
        ...
        <div class="collection_to">
            <ul class="discover-folders2">
                ...
                <li>
                    <h3>{{$value}}</h3>
                    <span img="" floder_id="{{$key}}" class="folderattr null" title="{{$value}}"></span> <a href=" " class="Button2 fr to_find_floder_act add_finder_btn" data-id="{{$key}}" data-img="" data-source="{{$news->id}}">收藏</a > 
                </li>
                @endforeach
            </ul>
        </div>
